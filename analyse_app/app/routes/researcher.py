...
from app.models import db, Cohort, AnalyseAudit
...
def _record_export_audit(*, export_id: str, cohort_id: str,
    ...
    """Write one ``AnalyseAudit`` row capturing the completed export.
...
def register_export_audit_cli(app) -> None:
    ...
    AnalyseAudit row per entry with route='research_export'.
    ...
    def migrate_export_audit_log(log_path, dry_run):
        ...
        existing = set()
        rows = (
            db.session.query(AnalyseAudit.payload_snapshot)
            .filter(AnalyseAudit.route == "research_export")
            .all()
        )
        for (snap,) in rows:
