"""SSO web login flow for analyse.pdhc (analysis-phase gated).

Ported from dashboard.pdhc/app/routes/auth.py (which itself mirrors
gateway.pdhc/app/web/auth.py). Provides the browser-facing
redirect → callback → session flow. On success the callback stores
``session['sso_token']``; the global before_request loader in ``app.auth``
re-validates that token against sso.pdhc on EVERY subsequent request
(Rule 11 — no blob cache), so an SSO-side logout locks the caller out
immediately. The callback admits only callers for whom
``has_analysis_access`` is True (SU admin OR professional with the 'analysis'
phase).

These ``/auth/*`` routes are PUBLIC (``app.auth._public_path``) so the login
handshake itself is reachable without a session.
"""
...
def login():
    ...
    next_url = (
        ...
        or session.pop("sso_next", None)
        or url_for("views.landing")
    )
...
<head>
<meta charset="utf-8">
<title>Logged out — analyse.pdhc</title>
<style>
