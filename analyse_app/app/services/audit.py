...
call produces exactly one ``AnalyseAudit`` row, including 4xx denials.
...
from app.models import AnalyseAudit, db
...
def audit_read(fn):
    """Wrap a Flask view so each call writes one ``AnalyseAudit`` row.
...
def _write_audit_row(*, status: int, n_rows: int | None) -> None:
    """Insert a single AnalyseAudit row. Never raises — a failed
    ...
    try:
        ...
        try:
            ...
            pass
        try:
            current_app.logger.warning("analyse_audit write failed: %s", exc)
        except Exception:
            pass
